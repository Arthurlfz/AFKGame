// 任务系统回归测试（123 条任务 v5：13 新手 + 68 主线 + 12 日常 + 6 成就 + 24 宠物）
// 注：2026-08-30 地图从 6 图扩到 10 图，主线由 24 条（6图×4）扩到 40 条（10图×4）；
//     2026-08-31 新增宠物专属 24 条（8 宠 × 3 养成链：孵化→带它击杀→它进化），独立 pet 分类
//     2026-09-01 新手链 +1：t9 改 synth 类型（教程第9章合成）+ 新增 t13 魂铸（教程第12章）→ 13 条新手
...
(async () => {
  ...
  A(JSON.parse(types).length === 13, '覆盖 13 种任务类型（实际 ' + JSON.parse(types).length + ' 种）');

  /* ---------- 新手链前置依赖 ---------- */
  A(C(`Config.drop.quests.find(q=>q.id==='t1') && !Config.drop.quests.find(q=>q.id==='t1').requires`), '新手第一条 t1 无前置');
  A(C(`['t2','t3','t11','t12','t4','t5','t6','t7','t8','t9','t10','t13'].every(id=>!!(Config.drop.quests.find(q=>q.id===id)||{}).requires)`), '新手 t2~t13 都配了前置任务');
  ...
  A(C(`(function(){const T=Config.drop.quests.filter(q=>q.category==='tutorial');
    ...
    return n===T.length})()`), '新手链线性完整：单起点、无断链、无分叉、无环，13 条全串起来');
  ...
  // 2026-09-02 引导强化：引导段任务（isGuide）都要有 hint（怎么做）和 target（高亮元素）
  A(C(`Config.drop.quests.filter(q=>q.category==='tutorial'&&q.isGuide).every(q=>q.hint && q.target)`), '引导段任务（isGuide）都配了 hint（怎么做）和 target（高亮元素）');
  A(C(`['t1','t4','t9','t10'].every(id=>!!(Config.drop.quests.find(q=>q.id===id)||{}).target)`), '关键引导任务 t1/t4/t9/t10 都有高亮 target');
  A(C(`!Config.drop.quests.find(q=>q.id==='t13').target`), '非引导段 t13 不配高亮 target');
