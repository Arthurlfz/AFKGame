...
// ===== 3) 地图专属材料 + 进化素材掉落 =====
const matDiag=await C(`(async()=>{const area={id:"corrupted-forest",name:"枯荣之地"};const e={name:"测试怪",level:1,tier:"common"};let guard=0,gotArea=false,gotEvo=false;while((!gotArea||!gotEvo)&&guard<100){const r=await Drop.rollReward(e,area);if(r&&r.type==="material"&&r.material==="枯荣种荚")gotArea=true;if(r&&r.type==="material"&&(r.material==="进化素材"||r.material==="精粹进化素材"||r.material==="传说进化素材"))gotEvo=true;guard++;}return {gotArea,gotEvo,guard,areaNow:Materials.getQuantity("枯荣种荚"),evoNow:Materials.getQuantity("进化素材")}})()`);
