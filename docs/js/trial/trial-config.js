...
(function () {
  ...
  window.Config.resourceTrials = {
    enabled: true,
    /* 服务端权威开关（2026-09-23）：true = 成败与奖励由 Edge Function `resource-trial` 判定，
     * 客户端只播结果（不扣票、不发奖）。**关掉即回到旧的本地算法**（应急用）。
     * ⚠️ 关掉之后，免费次数又回到本地 localStorage —— 两条路的账本不同，来回切会出现
     *    "今天免费次数看起来不对"，只应在出问题时临时关，不要长期混着用。
     *
     * 🔴 2026-09-23 晚曾回滚为 false（用户实机反馈：怪物看不见、整局几秒就打完）。
     *   原因（**回放层没做完**，不是服务器算错）：
     *     ① 服务器模式没有实时战斗引擎 ⇒ 战斗区的怪物/宠物**根本没被初始化**（只写了层数那行字）；
     *     ② 回放只有"逐层播报"，没有每层的出手演出 ⇒ 观感上"点了就结束"。
     * ✅ 已补齐并**重新打开**：
     *     · `js/trial/trial-replay.js`：每层 `UI.resetBattle` 摆怪物 + 照服务器给的出手序列
     *       播（出手动作 / 伤害飘字 / 血条），一层 1.2~4 秒自动压缩；
     *     · 服务器每层的 `events`（谁出手、伤害、暴击/闪避）由 trial-core 一并返回；
     *     · 守值 `vtest_trial_server.js` 断言"每层都摆了怪物 + 有出手与飘字"，`vtest_trial_core.js`
     *       断言"服务器每层都带演出数据" —— 少一样就红。
     * ⚠️ 再出问题就把它改回 false（一行应急），并把现象告我。 */
    serverAuthority: true,
    ticketName: '资源试炼门票',
    /* 每副本每日免费进入次数（北京时间 12:00 刷新，见 trial-access.js 的 dayKeyOf）。
     * 免费次数用尽后进入需消耗 1 张门票。设为 0 即回到纯门票模式。
     * ⚠️ 2026-09-16 用户拍板「副本重置不该这么容易拿到」：3 → 1。
     *   3 是【按路线各算】的（trial-access 的 usage.used[routeId]）⇒ 3 条路线 = 每天白打 9 局；
     *   改成 1 后 = 每天 3 局免费（三条路线各试一次），想多刷必须拿门票。
     *   门票的主来源是「交地图委托」（每轮 1 张），兑换只做兜底。 */
    freeEntriesPerDay: 1,
     ...
     *   「当前毕业档（成长 100）+ 主流装备」打得过但紧张（校准记录见 tests/vtest_resource_trial.js）。 */
    floors: 20,
    floorLevelStart: 10,
    floorLevelEnd: 100,
    ...
    eliteEvery: 5,
    eliteMult: 1.1,
    /* 2026-09-15 重校：新装备 T1% 乘全属性 → 毕业战力上抬，旧基准下副本通关剩血 72%（太轻松）。
     * 血量 ×1.5 / 攻 ×1.24 后实测：低档(g30 裸) 7 层、中档(g60 中装) 12 层、毕业(g100 满金) 通关剩血 25%
     * —— 与旧锚点「打得过但紧张（剩血 20~50%）」对齐。改参数必须重跑副本模拟。 */
    baseStats: { hp: 150000, atk: 8900, def: 3600 },
    /* 守关者机制属性（2026-09-15 命中/闪避升格）：守关者也有命中/闪避，跟怪等级（Lv10→100）走。
     *   命中 = hitPerLv × 怪等级；闪避 = dodgeAtRef × (怪等级 / refLevel)^dodgeExp
     * （旧 guardianHit:160 写死值退役。改参数必须重跑副本难度校准。） */
    mech: { hitPerLv: 3.5, dodgeAtRef: 200, refLevel: 60, dodgeExp: 1.6 },
    /* 【难度校准记录 2026-09-10，BattleSim 蒙特卡洛 500 场/层】
     * ⚠️ 2026-09-23 更正措辞：**成长没有上限**（2026-09-17 用户拍板取消软上限 100；
     *    DB 只剩防脏数据的极宽上界 growth ≤ 1000000，见 `supabase/migrate_growth_no_cap.sql`）。
     *   下文「成长 100」只是当年校准用的**一档参考宠**（旧口径把它当天花板，现已作废），
     *   **不是天花板** —— 成长更高的玩家只会更轻松。
     * 参考宠 = Lv60 + 成长 100 + 12 件金装(T10/底材T5)（面板 攻1.45万/血3.0万/防6184/闪避46-88/吸血14-22%）：
     *   第20层胜率 100% 但单层掉血 30%（6.6刀）——叠加血量跨层累计，实战通关剩余血 ~20-50%，紧张但能过；
     * 成长30裸装（攻4306）：≥50%胜率最深第 9 层；成长60金装（攻8806）：最深第 19 层 —— 曲线按成长"以此类推"成立。
     * 注意：成长 100 的玩家对野图顶格数值（图10 血2520）是一刀秒（引擎注释"涅槃叠成长→一刀秒"），
     *   所以副本锚点必须远超野图顶格 —— 这就是"顶层=毕业宠+装备才拿得下"的物理含义。
     * 🔴 待用户拍板（成长无上限之后的新问题）：副本/塔的难度锚点仍按「成长 100 档」定，
     *   而成长 300+ 的玩家现在进副本是碾压 —— 要不要按更高的成长重新定标，等发话。 */
   ...
   * 🔴 2026-09-16 用户纠正（「就是这个地图委托太容易兑换了」）：**委托的需求量是这张票的唯一闸门** ——
   *   原来每轮只要 50 个区域材料（≈2 小时挂机）就换 1 张门票，而 1 张门票 = 1 局副本（1~3 分钟），
   *   挂一天能刷十几局 ⇒ 副本的"限定次数"形同虚设。已把 10 条委托的需求统一提到 **200**（≈8.3 小时 1 张）。
   *   ⚠️ 这里注入的只是"给什么"，"要多少"在 `core/config.js` 的 quests 表里 —— 两者要一起看。
   *   ⚠️ 它仍是**唯一稳定来源**（每日兑换「门票熔铸」与周常 wk5 的门票已于同日删/改），别再加第二个来源。 */
  (function () {
