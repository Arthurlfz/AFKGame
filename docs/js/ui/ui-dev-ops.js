...
(function () {
  ...
  // 条形分布：宽度按最大值归一
  function bars(rows) {
    if (!rows.length) return '<div class="dev-hint">暂无数据</div>';
  ...
  async function render(host) {
    host.innerHTML = '<div class="dev-hint">正在读取…</div>';
    if (!window.Supabase || !window.Supabase.opsBoard) {
      host.innerHTML = '<div class="dev-hint">未就绪（Supabase.opsBoard 不存在）</div>';
      return;
    }
    const { data, error } = await window.Supabase.opsBoard();
    if (error) {
    ...
    host.innerHTML =
      '<div class="ops-head">内测看板<span class="hint">每次进这一页现拉一次</span></div>'

      + '<div class="ops-card"><div class="ops-title">活人</div><div class="ops-row">'
        + stat('账号总数', acc.total)
  ...
  DP.registerTab('ops', { render: render });
})();
