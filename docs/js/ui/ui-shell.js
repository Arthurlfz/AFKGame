...
(function () {
  ...
  const PAGES = new Set(['worldmap', 'battle', 'pet', 'bag', 'equip', 'market', 'market-sell']);
  ...
  function renderPage(page) {
    ...
    btns.forEach(b => {
      // 战斗页（page='battle'）从世界地图进入，侧边栏让「世界地图」也保持高亮，避免空指示
  ...
  // 从当前 hash 解析出目标页（非法/空 → 默认世界地图页）
  function pageFromHash() {
    const h = (location.hash || '').replace('#', '');
    return PAGES.has(h) ? h : 'worldmap';
  }
  ...
  function initShell() {
    ...
    // 刷新（F5 / 直接打开）默认停在世界地图（第二菜单），不记忆上次停的战斗页（第三菜单）。
    // 注意：前进/后退仍由 hashchange 驱动走 pageFromHash；这里仅控制"首次进入"的落点。
    renderPage('worldmap');
    // 若地址栏带着 #battle 之类内部 hash，刷新后统一落到世界地图（避免直接停进第三菜单）
    if (location.hash && location.hash !== '#worldmap') {
      history.replaceState(null, '', location.pathname + location.search + '#worldmap');
    }
  }
