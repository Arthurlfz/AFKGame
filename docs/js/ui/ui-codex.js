...
(function () {
  ...
  function buildEquipCraft() {
    ...
    return note(`装备共 ${slots.length} 个部位：${slots.map(s => escapeHtml(s)).join('、')}。每件装备由【基底（部位固定值）】与【词缀（前缀≤3 + 后缀≤3，共≤6 条）】两部分组成。`)
      ...
      + rules([
        ...
        '穿透（T1）= 无视 X% 防御（有效防御打折，上限 80%，越打高防怪越赚）；穿透（T2~T5）= 无视 X 点防御（常数，浅图有用、深图衰减）；伤害加成 = 最终伤害 +X%；受伤减免 = 受到伤害 -X%（最低承伤 10%）'
      ])
      // 打造石头
  ...
  function buildGrowth() {
    ...
    const rows = [
      ...
      ['<svg class="eic" viewBox="0 0 24 24" aria-hidden="true"><path d="M15.914 4a1.5 1.5 0 00-2.474-1.561l-9 9A1.5 1.5 0 005.5 14h4.002a.5.5 0 01.471.666L8.086 20a1.5 1.5 0 002.475 1.56l9-9A1.5 1.5 0 0018.5 10h-3.997a.5.5 0 01-.472-.667z"/></svg> 神级宠', `终阶（第 ${(SY.god && SY.god.minStage) || 5} 阶）+ Lv.${godLv}`,
        ...
        `门槛：主宠与副宠都终阶且等级达标，其中「主宠」成长 ≥ ${godMin}（2026-09-20 起副宠不看成长，达标即可当燃料）；神宠可用天仙玉露 / 琼浆玉露培育成长（成长无上限），血脉特质靠涅槃喂副宠慢慢烙（锁魂玉可定向）`],
  ...
  const ENTRIES = [
    ...
    { id: 'pet', icon: '<svg class="eic" viewBox="0 0 24 24" aria-hidden="true"><path d="M5.2 10.8a2 2 0 1 0 0-4 2 2 0 0 0 0 4z"/><path d="M12 8.6a2.6 2.6 0 1 0 0-5.2 2.6 2.6 0 0 0 0 5.2z"/><path d="M18.8 10.8a2 2 0 1 0 0-4 2 2 0 0 0 0 4z"/><path d="M12 13.2c-3 0-4.8 1.7-4.8 3.9 0 2.4 1.8 4.4 4.8 4.4s4.8-2 4.8-4.4c0-2.2-1.8-3.9-4.8-3.9z"/></svg>', title: '宠物成长', intro: '属性由基础值、等级、成长值和成长系数共同决定，速度是固定值。', build: buildPet },
