 ...
 *  · 🔴 只消费 bot.*，**不把云端数值段应用到本地**（2026-09-16 明确，别再"补上"）。
 *    原因：普通玩家的服务端结算读的是代码内置的 supabase/functions/_shared/config-server.mjs
 *    （battle-settle 只对管理员账号合并云端配置）。客户端若单方面应用云端数值，就会变成
 *    「界面按新数值、结算按旧数值」—— 经验条与掉落当场对不上。
 *    数值要让全员生效，正确路径是改 config.js → 重跑 supabase/gen_server_config.js → 部署 EF，
 *    不是在这里加白名单。
 *
