...
const report={starterCount:starters.length,allStarters:starters.length===8&&starters.every(x=>Array.isArray(tree[x.name])&&tree[x.name].length===2),lineCount:lines.length,uniqueTerminals:new Set(terminals).size,branchIndependent:lines.every(x=>x.first.to!==x.second.to&&x.second.to!==x.third.to),levels:lines.every(x=>x.first.minLevel===10&&x.second.minLevel===25&&x.third.minLevel===60),allTerminalUnique:new Set(terminals).size===16};
A(report.starterCount===8&&report.allStarters,'8 只基宠均在 tree 中且各有 2 条分支');A(report.lineCount===16,'共解析出 16 条三段分支');A(report.allTerminalUnique,'16 个第3阶终极宠全部互不重复');A(report.branchIndependent,'每条分支三段形态连续且无分支内汇合');A(report.levels,'每条分支 minLevel 严格为 10/35/60');
const skills=C('Config.pet.evolution.activeSkills');const skillReport={count:Object.keys(skills).length,allTerminalsConfigured:terminals.every(name=>{const s=skills[name];return s&&s.id&&s.name&&s.minLevel===60&&s.cooldownTurns===3&&typeof s.damageMultiplier==='number'&&s.damageMultiplier>1}),noExtraSkills:Object.keys(skills).every(name=>terminals.includes(name))};A(skillReport.count===16,'16 个终形态均配置主动技能');A(skillReport.allTerminalsConfigured,'每条主动技能均为 Lv60 解锁、3 回合冷却且具备伤害倍率');A(skillReport.noExtraSkills,'主动技能配置与终形态严格一一对应');A(skills['剧毒魔君'].maxHpDamageRate===0.03&&skills['剧毒魔神'].maxHpDamageRate===0.03,'剧毒系两条技能均附加目标最大生命 3% 伤害');
