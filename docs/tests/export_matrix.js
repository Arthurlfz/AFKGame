...
// 🔴 2026-09-17：进化素材三档改成**独立键**（`areaEvolutionTiers` + 占位键机制已删），
//   所以本函数不再需要任何"拆槽"计算 —— **表里只剩「区域材料」一个占位键要解析**，
//   其余键（含三档进化素材）本身就是真物品名。
...
const data = {
  ...
  // 每张图的进化素材三档分布（矩阵页要讲清"三档现在都是独立键、各自有权重"）
  evoSlots: areas.map(a => {
    const w = MWBT[a.tier] || {};
    const got = EVO_TIERS.filter(n => (Number(w[n]) || 0) > 0);
    const sum = got.reduce((s, n) => s + (Number(w[n]) || 0), 0) || 1;
    const share = {};
    got.forEach(n => { share[n] = (Number(w[n]) || 0) / sum; });
    return { tier: a.tier, allowed: got, share: share, sum: sum };
  }),
