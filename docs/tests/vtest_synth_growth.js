 ...
 *   5. 普通宠成长**无上限**（2026-09-17 用户拍板取消软上限 100）
...
(async () => {
  ...
  /* 普通宠成长**无上限**（2026-09-17 用户拍板取消软上限 100）。
   * 同一次调用在旧规则下是 100 + (109-100)/2 = 104.5；现在应直接是 109（不对超出部分减半）。
   * 阈值取 104.6 就是为了区分这两种规则：低于它说明"减半"又回来了。 */
  ...
  C('Math.random=__r5');
  A(g5 > 104.6, `普通宠成长无上限（95 + 提升 → ${g5}，不再对超 100 的部分减半；旧规则会停在 104.5）`);
