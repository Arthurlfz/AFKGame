...
/* ===== 曲线底线（2026-09-16 策划批准降速后重定） =====
 * 背景：玩家反馈「升级太简单」—— 旧曲线从头到尾 2~3 分钟一级，1→60 只要 2.4 小时。
 * 现在整体拉长到约 7.8 小时（前期 ~1.5 分钟一级、后期 ~11 分钟一级），4 条底线同步放宽：
 *   · Lv1 ≤ 10 场（≈1.7 分钟）：前期**仍然保留连续升级的爽感**；
 *   · Lv30 / Lv60 上限放宽：承认后期会"磨"一点，这正是"等级有分量"的代价；
 *   · 单场进度下限 3% → 1%：旧底线成立的前提是**玩家只靠进度条位移获得反馈**
 *     （config 注释里写过：旧版 Lv30 每场涨 1.5% = 零反馈）。现在战斗页信息卡**同时显示
 *     "经验 2,341,282 / 3,472,282 · 29%"**，数字每场都在跳，反馈不依赖进度条位移。
 * ⚠️ 若哪天把信息卡的数字去掉，这条 1% 底线必须改回 3%。 */
A(fightsAt(1) <= 10, `Lv1 升级够快（${fightsAt(1).toFixed(1)} 场/级，前期要有连续升级的爽感）`);
A(fightsAt(30) <= 55, `Lv30 不磨（${fightsAt(30).toFixed(1)} 场/级）`);
A(fightsAt(60) <= 75, `Lv60 不磨（${fightsAt(60).toFixed(1)} 场/级）`);
let minPct = 100;
for (let lv = 1; lv <= 60; lv++) minPct = Math.min(minPct, 100 / fightsAt(lv));
A(minPct >= 1, `全等级单场进度或数字可见（最小 ${minPct.toFixed(1)}% ≥ 1%，另有每场数字跳变兜底）`);
