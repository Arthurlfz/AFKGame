...
(function () {
  ...
  window.Config.tower = {
    ...
    /* ⚠️ 2026-09-16 核实：下面三个字段是**预留（死配置）**，商品从未在 `products` 表上架，玩家买不到。
     * 而且 2026-09-12 已拍板「魔石只卖便利、不卖数值」—— 重置卡 = 多打一局 = 产装备/材料，**属数值 ⇒ 不该上架**。
     * 保留字段只为「将来接官方支付后重定价」时有锚，且 `vtest_tower.js` 断言 priceGems/limitPerWeek > 0。
     * ⇒ **玩家拿到它的唯一路 = 每周兑换「塔券铸成」**（`ex_week_reset_card`，24 个腐印）。
     * 边界基线（档案 §3「通天塔重置卡 = 付费购买、禁止掉落/任务」）与现状的偏差记在
     * `docs/资源归属矩阵现状.md`，别再照基线文案写 UI。 */
    resetCard: {
    ...
    floorTiers: [
      ...
      { floor: 30, gear: { rarity: 'gold', count: 5, areaTier: 10, materialTier: 1 }, items: [{ name: '越龙之石', qty: 3 }, { name: '天仙玉露', qty: 3 }, { name: '强化丹B', qty: 3 }, { name: '琼浆玉露', qty: 1 }, { name: '幽冥经验髓', qty: 1 }] }
    ],
